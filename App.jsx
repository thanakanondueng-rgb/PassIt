...
function today(){return new Date().toLocaleDateString("th-TH",{day:"numeric",month:"short",year:"numeric"})}
...
export default function App(){
 ...
 const exportData=()=>{
   const blob=new Blob([JSON.stringify({subjects,tasks,history,examDate,emergency},null,2)],{type:"application/json"});
 ...
 return <div className="app">
  ...
  <main>
   ...
   {page==="home"&&<Dashboard score={score} days={daysTo(examDate)} subjects={subjects} tasks={tasks} completed={completed} totalMinutes={totalMinutes} totalPages={totalPages} emergency={emergency} setEmergency={setEmergency} toggleTask={toggleTask} onAI={()=>setAi(true)} onPomodoro={()=>setPomodoro(true)}/>}
...
function Countdown({days,score,emergency,setEmergency}){
 const [now,setNow]=useState(Date.now());
 useEffect(()=>{const x=setInterval(()=>setNow(Date.now()),1000);return()=>clearInterval(x)},[]);
 const target=new Date("2026-09-20T23:59:59").getTime(),diff=Math.max(0,target-now);
 const d=Math.floor(diff/86400000),h=Math.floor(diff%86400000/3600000),m=Math.floor(diff%3600000/60000),s=Math.floor(diff%60000/1000);
 return <section className="card countdown"><div className="count-copy"><h2>Countdown to Exam</h2><b>**สอบปลายภาค ภาคเรียนที่ 1/2569**</b><p>20 กันยายน 2569</p><div className="timer">{[[d,"วัน"],[h,"ชั่วโมง"],[m,"นาที"],[s,"วินาที"]].map(x=><div><strong>{String(x[0]).padStart(2,"0")}</strong><small>{x[1]}</small></div>)}</div><div className="read-head"><span>ความพร้อมสอบ <b>{score}%</b></span><span>Exam Readiness Score</span></div><div className="track"><i style={{width:score+"%"}}/></div>{emergency&&<div className="warning">⚡ Emergency Mode เปิดอยู่ — เน้นวิชาสำคัญก่อน</div>}<button className="outline" onClick={()=>setEmergency(!emergency)}>{emergency?"ปิด Emergency Mode":"เปิด Emergency Mode"}</button></div><div className="alarm-big">⏰</div></section>
}

function Dashboard({score,days,subjects,tasks,completed,totalMinutes,totalPages,emergency,setEmergency,toggleTask,onAI,onPomodoro}){
 return <div className="content"><div className="grid"><div className="left">
  <Countdown days={days} score={score} emergency={emergency} setEmergency={setEmergency}/>
  <section className="card"><Title text="📅 แผนการเรียนวันนี้" right="6 กันยายน 2569"/><div className="table"><div className="row head"><span>วิชา</span><span>หัวข้อ</span><span>หน้า</span><span>เวลา</span><span></span></div>{tasks.map(t=><div className="row" key={t.id}><span><b className="dot">{subjects.find(s=>s.id===t.subjectId)?.icon}</b>{subjects.find(s=>s.id===t.subjectId)?.name}</span><span>{t.title}</span><span>{t.pages}</span><span>{t.time}</span><button className={"check "+(t.done?"on":"")} onClick={()=>toggleTask(t.id)}>{t.done&&<Check size={14}/>}</button></div>)}</div><button className="purple" onClick={onPomodoro}>เริ่มอ่าน 25 นาที <Play size={14}/></button></section>
  <div className="two"><ProgressCard subjects={subjects} score={score}/><Upcoming/></div>
 </div><div className="right">
  <Widget days={days} subjects={subjects}/><Reminders/><section className="card ai"><Sparkles/><div><b>AI Study Assistant</b><p>มีเวลาแค่ 1 ชั่วโมง? ให้ AI ช่วยจัดลำดับสิ่งที่ควรอ่าน</p><button onClick={onAI}>ถาม AI Assistant <ChevronRight size={14}/></button></div></section>
 </div></div>
...
function Widget({days,subjects}){return <section className="card widget"><Title text="Countdown Widget" right="(ตัวอย่าง)"/><div className="phone"><div className="screen"><b>สอบปลายภาค</b><small>20 ก.ย. 2569</small><strong>{days}</strong><span>วัน</span><p>13 : 45 : 30</p><div className="mini">{subjects.slice(0,3).map(s=><div><b>{s.icon} {s.name}</b><div><i style={{width:s.progress+"%"}}/></div><small>{s.progress}%</small></div>)}</div></div></div><button className="outline full">ปรับแต่ง Widget</button></section>}
function Reminders(){return <section className="card"><Title text="นาฬิกาปลุกเตือน" right="+ เพิ่ม"/>{["09:00","11:00","15:00"].map((t,i)=><div className="rem"><AlarmClock/><div><b>{t}</b><small>{["เริ่มเรียนฟิสิกส์","พักและทบทวน 10 นาที","เริ่มเรียนคณิตศาสตร์"][i]}</small></div><input type="checkbox" defaultChecked/></div>)}<button className="outline full">ดูทั้งหมด</button></section>}
